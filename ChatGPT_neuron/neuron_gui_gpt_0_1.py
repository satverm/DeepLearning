#chatgpt result after asking some changes
...
class NeuronGUI:
    def __init__(self, master):
        ...
        # Create GUI elements
        self.input_slider = self.create_slider(20, 20, "Input:", self.update_input)
        self.weight_slider = self.create_slider(20, 60, "Weight:", self.update_weight)
        self.bias_slider = self.create_slider(20, 100, "Bias:", self.update_bias)

    def create_slider(self, x, y, label, command):
        label = tk.Label(self.master, text=label)
        label.place(x=x, y=y)
        slider = ttk.Scale(self.master, from_=-10, to=10, orient=tk.HORIZONTAL, command=command)
        slider.set(0)
        slider.place(x=x+60, y=y, width=150)
        return slider
